...
We are here to build a complete CI/CD pipeline in the cloud by setting up a web application on AWS and preparing the foundation for automation. This phase focuses on launching an EC2 instance, connecting through VS Code, installing Java + Maven, and generating a basic web app. It sets up the base environment so later phases can automate building, storing artifacts, and deploying the application reliably.
